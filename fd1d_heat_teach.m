...
%% approximation and the exact solution for each time step, and will output the
...
for nt = 1:n
    ...
    maxerr = max(maxerr,norm(error,inf));  %% >>> Question to MP: I don't see the 2-norm here? Am I mis-understanding?
    
    %% for efficiency, you can comment out plotting, error report and pause   
    %% NOTE that the animation requires the user to input any key
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%% the user must code these expressions %%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function v = rhsfun(x,t)    %% This is f in the equation u_t - u_{xx} = f
v = zeros(size(x));

function v = initfun(x)     %% This is the initial condition function
v = sin(pi*x);

function v = exfun(x,t)     %% This is the exact solution
